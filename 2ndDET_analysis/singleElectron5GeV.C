...
void singleElectron5GeV(const char *inputFile)
{
  // Setting for figures
  ...
  // Book histograms
  TH1* hTrackDeltaEta = new TH1D("hTrackDeltaEta","-1.0<#eta<1.0;#eta^{rec}-#eta^{gen};Number of tracks for electron",100, -1.0, 1.0);
  TH1* hTrackDeltaPhi = new TH1D("hTrackDeltaPhi","-1.0<#eta<1.0;#phi^{rec}-#phi^{gen};Number of tracks for electron",100, -1.0, 1.0);
  TH1* hTrackDeltaPt = new TH1D("hTrackDeltaPt","-1.0<#eta<1.0;(Pt^{rec}-Pt^{gen})/Pt^{gen};Number of tracks for electron",100, -0.1, 0.1);
  TH1* hTrackDeltaP = new TH1D("hTrackDeltaP","-1.0<#eta<1.0;(P^{rec}-P^{gen})/P^{gen};Number of tracks for electron",100, -0.1, 0.1);
    
  TH1* hElectronDeltaEta = new TH1D("hElectronDeltaEta","-1.0<#eta<1.0;#eta^{rec}-#eta^{gen};Number of electrons",100, -1.0, 1.0);
  TH1* hElectronDeltaPhi = new TH1D("hElectronDeltaPhi","-1.0<#eta<1.0;#phi^{rec}-#phi^{gen};Number of electrons",100, -1.0, 1.0);
  TH1* hElectronDeltaPt = new TH1D("hElectronDeltaPt","-1.0<#eta<1.0;(Pt^{rec}-Pt^{gen})/Pt^{gen};Number of electrons",100, -0.1, 0.1);
  TH1* hElectronEhadOverEem = new TH1D("hElectronEhadOverEem","-1.0<#eta<1.0;Ratio of Ehad over Eem;Number of electrons",100, 0., 2.0);
    
  TH1* hEFlowTrackDeltaEta = new TH1D("hEFlowTrackDeltaEta","-1.0<#eta<1.0;#eta^{rec}-#eta^{gen};Number of eflowtracks for electron",100, -1.0, 1.0);
  TH1* hEFlowTrackDeltaPhi = new TH1D("hEFlowTrackDeltaPhi","-1.0<#eta<1.0;#phi^{rec}-#phi^{gen};Number of eflowtracks for electron",100, -1.0, 1.0);
  TH1* hEFlowTrackDeltaPt = new TH1D("hEFlowTrackDeltaPt","-1.0<#eta<1.0;(Pt^{rec}-Pt^{gen})/Pt^{gen};Number of eflowtracks for electron",100, -0.1, 0.1);
  TH1* hEFlowTrackDeltaP = new TH1D("hEFlowTrackDeltaP","-1.0<#eta<1.0;(P^{rec}-P^{gen})/P^{gen};Number of eflowtracks for electron",100, -0.1, 0.1);
    
  TH1* hTowerDeltaEta = new TH1D("hTowerDeltaEta","-1.0<#eta<1.0;#eta^{rec}-#eta^{gen};Number of towers for electron",100, -1.0, 1.0);
  TH1* hTowerDeltaPhi = new TH1D("hTowerDeltaPhi","-1.0<#eta<1.0;#phi^{rec}-#phi^{gen};Number of towers for electron",100, -1.0, 1.0);
  TH1* hTowerDeltaEem = new TH1D("hTowerDeltaEem","-1.0<#eta<1.0;Eem^{rec}/E^{gen};Number of towers for electron",100, 0., 2.0);
  TH1* hTowerDeltaEhad = new TH1D("hTowerDeltaEhad","-1.0<#eta<1.0;Ehad^{rec}/E^{gen};Number of towers for electron",100, 0., 2.0);
  TH1* hTowerDeltaE = new TH1D("hTowerDeltaE","-1.0<#eta<1.0;(E^{rec}-E^{gen})/E^{gen};Number of towers for electron",100, -0.1, 0.1);
  ...
  for(entry = 0; entry < allEntries; ++entry)
  {
	...
	particle = (GenParticle*) electron->Particle.GetObject();
        ...
        if (particle->PID == 11)
        {
          hElectronDeltaEta->Fill((electron->Eta - particle->Eta));
          hElectronDeltaPhi->Fill((electron->Phi - particle->Phi));
          hElectronDeltaPt->Fill((electron->PT - particle->PT)/particle->PT);
          hElectronEhadOverEem->Fill(electron->EhadOverEem);
        }
      }
    }
    //
  ...
  //c5->SaveAs("./plots/hElectronDeltaEta.png");
  c2->cd(2);
  //TCanvas* c6 = new TCanvas("c6","c6");
  hElectronDeltaPhi->GetXaxis()->CenterTitle(true); 
  hElectronDeltaPhi->GetYaxis()->CenterTitle(true); 
  hElectronDeltaPhi->SetLineWidth(2);
  hElectronDeltaPhi->Fit("gaus");
  TF1* gaus_hElectronDeltaPhi = hTrackDeltaPhi->GetFunction("gaus");
  gaus_hElectronDeltaPhi->SetLineWidth(2);
  gaus_hElectronDeltaPhi->SetLineColor(kRed);  
  hElectronDeltaPhi->Draw();
  //c6->SaveAs("./plots/hElectronDeltaPhi.png");
  c2->cd(3);
  ...
  //c7->SaveAs("./plots/hElectronDeltaPt.png");
  c2->cd(4);
  //TCanvas* c8 = new TCanvas("c8","c8");
  hElectronEhadOverEem->GetXaxis()->CenterTitle(true); 
  hElectronEhadOverEem->GetYaxis()->CenterTitle(true); 
  hElectronEhadOverEem->SetLineWidth(2);
  hElectronEhadOverEem->Draw();
  //c8->SaveAs("./plots/hElectronEhadOverEem.png");
  c2->SaveAs("./plots/hElectronSummary.png");
