...
void singleMuon5GeV(const char *inputFile)
{
  // Setting for figures
  ...
  // Book histograms
  TH1* hTrackDeltaEta = new TH1D("hTrackDeltaEta","-1.0<#eta<1.0;#eta^{rec}-#eta^{gen};Number of tracks for muon",100, -1.0, 1.0);
  TH1* hTrackDeltaPhi = new TH1D("hTrackDeltaPhi","-1.0<#eta<1.0;#phi^{rec}-#phi^{gen};Number of tracks for muon",100, -1.0, 1.0);
  TH1* hTrackDeltaPt = new TH1D("hTrackDeltaPt","-1.0<#eta<1.0;(Pt^{rec}-Pt^{gen})/Pt^{gen};Number of tracks for muon",100, -0.1, 0.1);
  TH1* hTrackDeltaP = new TH1D("hTrackDeltaP","-1.0<#eta<1.0;(P^{rec}-P^{gen})/P^{gen};Number of tracks for muon",100, -0.1, 0.1);
  TH1* hTrackGenPID = new TH1D("hTrackGenPID", ";PID of Generated Particles associated with Track; Events",100,-100.,100.);
  TH1* hTrackRecPID = new TH1D("hTrackRecPID", ";PID of Reconstructed Particles associated with Track; Events",100,-100.,100.);
    
  TH1* hEFlowTrackDeltaEta = new TH1D("hEFlowTrackDeltaEta","-1.0<#eta<1.0;#eta^{rec}-#eta^{gen};Number of eflowtracks for muon",100, -1.0, 1.0);
  TH1* hEFlowTrackDeltaPhi = new TH1D("hEFlowTrackDeltaPhi","-1.0<#eta<1.0;#phi^{rec}-#phi^{gen};Number of eflowtracks for muon",100, -1.0, 1.0);
  TH1* hEFlowTrackDeltaPt = new TH1D("hEFlowTrackDeltaPt","-1.0<#eta<1.0;(Pt^{rec}-Pt^{gen})/Pt^{gen};Number of eflowtracks for muon",100, -0.1, 0.1);
  TH1* hEFlowTrackDeltaP = new TH1D("hEFlowTrackDeltaP","-1.0<#eta<1.0;(P^{rec}-P^{gen})/P^{gen};Number of eflowtracks for muon",100, -0.1, 0.1);
  TH1* hEFlowTrackGenPID = new TH1D("hEFlowTrackGenPID", ";PID of Generated Particles associated with EFlowTrack; Events",100,-100.,100.);
  TH1* hEFlowTrackRecPID = new TH1D("hEFlowTrackRecPID", ";PID of Reconstructed Particles associated with EFlowTrack; Events",100,-100.,100.);
    
  TH1* hTowerDeltaEta = new TH1D("hTowerDeltaEta","-1.0<#eta<1.0;#eta^{rec}-#eta^{gen};Number of towers for muon",100, -1.0, 1.0);
  TH1* hTowerDeltaPhi = new TH1D("hTowerDeltaPhi","-1.0<#eta<1.0;#phi^{rec}-#phi^{gen};Number of towers for muon",100, -1.0, 1.0);
  TH1* hTowerDeltaEem = new TH1D("hTowerDeltaEem","-1.0<#eta<1.0;Eem^{rec}/E^{gen};Number of towers for muon",100, 0., 2.0);
  TH1* hTowerDeltaEhad = new TH1D("hTowerDeltaEhad","-1.0<#eta<1.0;Ehad^{rec}/E^{gen};Number of towers for muon",100, 0., 2.0);
  TH1* hTowerDeltaE = new TH1D("hTowerDeltaE","-1.0<#eta<1.0;(E^{rec}-E^{gen})/E^{gen};Number of towers for muon",100, -0.1, 0.1);
  ...
  for(entry = 0; entry < allEntries; ++entry)
  {
    ...
    if (branchTrack->GetEntries() > 0)
    {
      ...
      for (i = 0; i < branchTrack->GetEntries(); ++i)
      {
        ...
        hTrackGenPID->Fill(particle->PID);
        hTrackRecPID->Fill(track->PID);
    ...
    if (branchEFlowTrack->GetEntries() > 0)
    {
      for (i = 0; i < branchEFlowTrack->GetEntries(); ++i)
      {
        ...
        hEFlowTrackGenPID->Fill(particle->PID);
        hEFlowTrackRecPID->Fill(eflowtrack->PID);
  ...
  hTrackDeltaP->Draw();
  c1->cd(5);
  hTrackGenPID->GetXaxis()->CenterTitle(true);
  hTrackGenPID->GetYaxis()->CenterTitle(true);
  hTrackGenPID->SetLineWidth(2);
  hTrackGenPID->Draw();
  c1->cd(6);
  hTrackRecPID->GetXaxis()->CenterTitle(true);
  hTrackRecPID->GetYaxis()->CenterTitle(true);
  hTrackRecPID->SetLineWidth(2);
  hTrackRecPID->Draw();
  ...
  c3->cd(5);
  hEFlowTrackGenPID->GetXaxis()->CenterTitle(true);
  hEFlowTrackGenPID->GetYaxis()->CenterTitle(true);
  hEFlowTrackGenPID->SetLineWidth(2);
  hEFlowTrackGenPID->Draw();
  c3->cd(6);
  hEFlowTrackRecPID->GetXaxis()->CenterTitle(true);
  hEFlowTrackRecPID->GetYaxis()->CenterTitle(true);
  hEFlowTrackRecPID->SetLineWidth(2);
  hEFlowTrackRecPID->Draw();
